...
export default class Metadata {
  private static getAndDecodeNote(txRecord: unknown) {
  ...
  private static async getAsaCreationTx(indexer: Indexer, assetId: number) {
   ...
   * @returns number
   */
  static async getAssetIdByMetadataHash(
    client: Algodv2,
    hash: string,
    creator: string
  ): Promise<number> {
    const assets = (await Assets.getCreatedAssets(
      client,
      creator
    )) as AssetInfo[]
  ...
  static async getAssetMetadata(
    ...
    const createTx = await Metadata.getAsaCreationTx(indexer, assetId)
    return Metadata.getAndDecodeNote(createTx)
  }
}
